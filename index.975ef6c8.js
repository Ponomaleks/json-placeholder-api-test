...
function renderUsers(list) {
    const markupArray = list.map(({ id , name , username , phone , website , email  })=>{
...
const editCard = async (id)=>{
    ...
    Object.keys(user).forEach((key)=>{
        if (key !== 'id' && key !== 'company' && key !== 'address') document.getElementById(key).value = user[key];
    });
};
[
    ...form.elements
...
const deleteCard = async (id)=>{
    ...
    users.splice(users.findIndex((el)=>el.id === id
    ), 1);
    renderUsers(users);
    loader.classList.remove('show-loader');
};
...
},{}]},["7nZVA","8lqZg"], "8lqZg", "parcelRequirefb46")
